import sys
import os

# Add src directory to Python path
src_dir = os.path.dirname(os.path.abspath(__file__))
if src_dir not in sys.path:
  sys.path.insert(0, src_dir)

from data.config import SETTINGS
from data.connector.connector import connect_to_mongo
from data.fetch_historical_daily import upsert_daily_history
from features.build_features import build_features, SYMBOLS
from data.store_features import store_features

import traceback


def main():
  """Main function to fetch and store historical cryptocurrency data."""
  print("=" * 60)
  print("Cryptocurrency Data Collection")
  print("=" * 60)

  try:
    # Connect to MongoDB
    print(f"\nConnecting to MongoDB at {SETTINGS['MONGO_HOST']}:{SETTINGS['MONGO_PORT']}...")
    mongo_client = connect_to_mongo(
      db_name=SETTINGS["MONGO_DB"],
      host=SETTINGS["MONGO_HOST"],
      port=int(SETTINGS["MONGO_PORT"]),
      user=SETTINGS.get("MONGO_USER", ""),
      password=SETTINGS.get("MONGO_PASSWORD", ""),
      auth=bool(SETTINGS.get("MONGO_USER", ""))
    )

    # Test MongoDB connection
    mongo_client[SETTINGS["MONGO_DB"]].list_collection_names()
    print(f"✓ Connected to MongoDB database: {SETTINGS['MONGO_DB']}")

    # Fetch and upsert historical daily data
    print("\n" + "=" * 60)
    print("Fetching historical data from Binance...")
    print("=" * 60)
    upsert_daily_history()

    # Compute technical indicators and store them in PostgreSQL
    # (mirrors the "compute_features" task of the Airflow DAG fetch_and_store)
    print("\n" + "=" * 60)
    print("Computing features and storing them in PostgreSQL...")
    print("=" * 60)
    for symbol in SYMBOLS:
      print(f"\n{symbol}: building features...")
      df = build_features(symbol)
      if df.empty:
        print(f"{symbol}: no OHLCV data available — features skipped")
        continue
      stored = store_features(df, symbol)
      print(f"{symbol}: {stored} rows stored in PostgreSQL")

    print("\n" + "=" * 60)
    print("✅ Data collection completed successf